...
(function () {
    ...
    const panelMover = document.getElementById('panel-mover');
    const panelElim  = document.getElementById('panel-eliminar');

    function getChks() { return document.querySelectorAll('.chk-registro'); }

    function actualizarEstadoMaestro() {
        const sel  = document.querySelectorAll('.chk-registro:checked').length;
        const todo = getChks().length;
        if (chkTodos) {
            chkTodos.checked       = sel > 0 && sel === todo;
            chkTodos.indeterminate = sel > 0 && sel < todo;
        }
    }

    document.querySelectorAll('input[name="tipo_accion"]').forEach(r => {
        r.addEventListener('change', function () {
            panelMover.classList.toggle('d-none', this.value === 'eliminar');
            panelElim.classList.toggle('d-none',  this.value !== 'eliminar');
        });
    });
    ...
    document.querySelectorAll('.fila-registro').forEach(fila => {
        fila.addEventListener('click', function (e) {
            if (e.target.type === 'checkbox') {
                actualizarEstadoMaestro();
                return;
            }
            const chk = fila.querySelector('.chk-registro');
            if (chk) { 
    ...
    getChks().forEach(c => c.addEventListener('change', actualizarEstadoMaestro));
})();
</script>
